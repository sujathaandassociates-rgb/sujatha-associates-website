...
export default function WhyChooseUs() {
  return (
    <section className="relative overflow-hidden bg-[#14342f] py-24 text-white sm:py-28">
      <div className="absolute -left-24 top-10 h-80 w-80 rounded-full bg-[#b78a35]/10 blur-3xl" />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-16 px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-10">
        <div className="lg:sticky lg:top-32 lg:self-start">
          ...
          <Link
            href="/about"
            className="mt-9 inline-flex items-center gap-3 rounded-full border border-[#d6b56c]/60 px-7 py-4 text-sm font-semibold text-white transition duration-300 hover:-translate-y-0.5 hover:bg-[#d6b56c] hover:text-[#14342f]"
          >
        ...
        <div className="grid gap-px overflow-hidden rounded-[2rem] border border-white/10 bg-white/10 sm:grid-cols-2">
          {reasons.map((reason) => (
            <article
              key={reason.number}
              className="group min-h-[280px] bg-[#173b35] p-8 transition duration-300 hover:bg-[#1d463f] sm:p-10"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold tracking-[0.24em] text-[#d6b56c]">
                  {reason.number}
                </span>

                <span className="h-2 w-2 rounded-full bg-[#d6b56c] transition duration-300 group-hover:scale-150" />
              </div>
